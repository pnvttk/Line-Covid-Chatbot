...
def time_info(respond_dict):
    #ตัวแปรเก็บค่า time
    time = float(respond_dict["queryResult"]["outputContexts"][1]["parameters"]["time.original"])
    print(time)
    time_str = str(time)
    print(time_str)
...
def temp_info(respond_dict): #ฟังก์ชั่นสำหรับบันทึกข้อมูล
    temp = float(respond_dict["queryResult"]["outputContexts"][1]["parameters"]["condition.original"])
    print(temp)
    temp_str = str(temp)
    print(temp_info)
...
def covid_today():
    ...
    covid = json_data['Confirmed']
    #return covid
    data = json.dumps(covid)
    answer_function3 = data
    answer_function5 = 'ยอดติดเชื้อสะสม : ' + data
    #answer_function2 = '{"line":{"text": ' + data + ',"type": "text"} }'
    return answer_function5
